...
export type OpsExplainStuckResponse = {
  order_id: number;
  state_machine?: {
    current_state?: string;
    last_event?: { event?: string; at?: string };
    expected_next_states?: string[];
    allowed_transitions?: Array<{ from?: string; to?: string; rule?: string }>;
    blockers?: Array<{ code?: string; severity?: string; details?: string }>;
    suggested_ops_actions?: Array<{ action?: string; reason?: string }>;
  };
  explanation?: string;
  suggested_next_actions?: Array<{ action?: string; payload?: Record<string, any>; priority?: string }>;
  ai_meta?: Record<string, any>;
  rid?: string;
};

export async function opsExplainStuck(
  token: string,
  input: { orderId: number; includeAi?: boolean }
): Promise<OpsExplainStuckResponse> {
  const includeAi = input.includeAi ? 1 : 0;
  return apiFetch(`/ops/orders/${input.orderId}/ai/explain-stuck?include_ai=${includeAi}`, {
    method: "GET",
    token,
  });
}
