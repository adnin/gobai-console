...
import { adminCashouts, adminCashoutsApprove, adminCashoutsReject } from "@/features/admin/api/adminApi";
...
export function AdminCashoutsPage() {
  const { token } = useAuth();
  const qc = useQueryClient();

  const [status, setStatus] = React.useState<"hold" | "approved" | "rejected">("hold");
  const [q, setQ] = React.useState<string>("");

  const listQ = useQuery({
    queryKey: ["admin", "cashouts", status],
    queryFn: async () => adminCashouts(String(token), { status }),
    enabled: !!token,
    refetchOnWindowFocus: false,
  });

  const approveM = useMutation({
    mutationFn: async (id: number) => adminCashoutsApprove(String(token), id),
    onSuccess: async () => qc.invalidateQueries({ queryKey: ["admin", "cashouts"] }),
  });

  const rejectM = useMutation({
    mutationFn: async ({ id, reason }: { id: number; reason: string }) => adminCashoutsReject(String(token), id, reason),
    onSuccess: async () => qc.invalidateQueries({ queryKey: ["admin", "cashouts"] }),
  });

  const rows: any[] = Array.isArray((listQ.data as any)?.data)
    ? (listQ.data as any).data
    : Array.isArray((listQ.data as any)?.data?.data)
    ? (listQ.data as any).data.data
    : [];
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-4 p-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-xl font-semibold">Wallet Cash-outs</h1>
          <div className="text-sm text-muted-foreground">Approve payout requests (debits points from the wallet).</div>
        </div>
        <div className="flex items-center gap-2">
          <Button asChild variant="secondary" size="sm"><Link to="/admin">Back</Link></Button>
          <StatusPill active={status === "hold"} onClick={() => setStatus("hold")}>Hold</StatusPill>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="text-xs text-muted-foreground">
            Note: Cash-out approval is idempotent; repeated clicks should not double-debit points.
          </div>
        </CardContent>
      </Card>
    </div>
